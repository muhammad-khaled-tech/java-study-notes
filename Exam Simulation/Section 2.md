...
**الإجابة الصحيحة هي: C. true false**

### 💡 الشرح التفصيلي (Deep Explanation)

ده واحد من أشهر أسئلة الـ "Memory Optimization" في الجافا، واسمه **Integer Caching**.

1. ماذا يحدث في الكواليس؟
    
    لما بتكتب Integer a = 127;، الجافا بتعمل Autoboxing. يعني بتنادي ميثود اسمها Integer.valueOf(127).
    
2. قاعدة الـ Cache (The Rule):
    
    الجافا، عشان توفر في الميموري، قررت إن الأرقام "الصغيرة" اللي بنستخدمها كتير (من -128 لحد 127) يتم تخزينها مسبقاً في "مخزن" (Cache) أول ما البرنامج يشتغل.
    
    - **لو الرقم جوه الـ Range ده (زي 127):** الجافا بترجعلك عنوان (Reference) لنفس الـ Object الموجود في المخزن.
        
        - عشان كده `a` و `b` بيشاوروا على **نفس المكان** في الميموري -> `(a == b)` ترجع **true**.
            
    - **لو الرقم بره الـ Range ده (زي 128):** الجافا بتفهم إن ده رقم كبير أو مش شائع الاستخدام كـ Object، فبتعمل `new Integer(128)` جديد خالص في الـ Heap.
        
        - عشان كده `c` ده Object جديد، و `d` ده Object تاني خالص -> `(c == d)` ترجع **false**.
            

### ⚠️ خد بالك من التكات:

1. **`new` تكسر القاعدة:** لو كتبت `Integer a = new Integer(127);`، هنا أنت بتجبر الجافا تعمل Object جديد حتى لو الرقم جوه الـ Cache، والنتيجة هتبقى `false`.
    
2. **الـ equals:** لو استخدمت `.equals()` في الحالتين هترجع `true` لأنها بتقارن القيمة (Value) مش العنوان (Reference).
