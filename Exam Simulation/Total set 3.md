...
**What is the result?** A. A100 B. B0 C. Compilation Error: `return` not allowed. D. Compilation Error: `yield` not allowed.

**Answer: C (Compilation Error: `return` not allowed)**

**Deep Dive:** In a **Switch Expression** (a switch that returns a value), you act within a specific scope to compute a result for that variable.
...
**What is the output?** A. -15 B. 15 C. -5 D. The output is unpredictable.

**Answer: D (The output is unpredictable)**

**Deep Dive:** The `reduce` operation requires the accumulator function to be **Associative**.
...
**What occurs when compiling this record?** A. Compiles successfully. B. Compilation Error at Line X: Cannot assign to final field. C. Compilation Error at Line X: Name clash. D. Runtime Exception.

**Answer: B (Compilation Error at Line X)**

**Deep Dive:** This is a strict rule regarding **Compact Constructors** in Records.
...
**Which directive is required at Line X to allow consumers to use the `ServiceImpl` class via Reflection?** A. `exports com.impl;` B. `opens com.impl;` C. `requires transitive com.impl;` D. `uses com.impl.ServiceImpl;`

**Answer: B (`opens com.impl;`)**
...
**What is the output?** A. `../..` B. `c/d` C. `/c/d` D. `d/c`

**Answer: B (`c/d`)**

**Deep Dive:** `relativize(p2)` answers the question: "How do I get **to** `p2` **from** `p1`?"
...
**If an external process modifies the 'qty' of the current row while the ResultSet is open, what happens?** A. The new value is printed. B. The old value is printed. C. A `SQLException` is thrown. D. The behavior is undefined.

**Answer: A (The new value is printed)**

**Deep Dive:** The key lies in `ResultSet.TYPE_SCROLL_SENSITIVE`.
...
**What is the output?** A. `Hello` B. `World` C. `HelloWorld` D. `null`

**Answer: D (`null`)**

**Deep Dive:** The `merge` method has specific logic for removal:
...
**(Source: Chapter 8 - Concurrency / Lesson 10)**
...
**Is this code thread-safe?** A. Yes, because `AtomicInteger` methods are atomic. B. No, because there is a "Check-Then-Act" race condition. C. Yes, because `get()` creates a memory barrier. D. No, because `incrementAndGet` is not synchronized.

**Answer: B (No, "Check-Then-Act" race condition)**

**Deep Dive:** Although `count.get()` and `count.incrementAndGet()` are individually atomic, the **block** of logic is not.
...
**What happens at Line X?** A. Compiles successfully. B. Compilation Error: Generic array creation. C. Runtime ClassCastException. D. Creates an array of Objects.

**Answer: B (Compilation Error: Generic array creation)**

**Deep Dive:** Java strictly forbids creating arrays of a generic type parameter (`new T[]`).
...
**(Source: Chapter 4 - Exceptions / Lesson 6)**
...
**What is the output?** A. `Shell Room` B. `Room Shell` C. `Room` D. `Shell`
...
**(Source: Chapter 1 - Date/Time / Lesson Appendix)**
...
**What is the output?** A. October at 3 o'clock B. October at 3 o''clock C. October at 3 o clock D. Runtime Exception: Invalid pattern.

**Answer: A (October at 3 o'clock)**

**Deep Dive:** In `DateTimeFormatter` patterns:
...
**What is the content of the map?** A. `{A=[Apple, Apricot], B=[Banana]}` B. `{A=[APPLE, APRICOT], B=[BANANA]}` C. `{65=[APPLE, APRICOT], 66=[BANANA]}` D. Compilation Error.

**Answer: B (`{A=[APPLE, APRICOT], B=[BANANA]}`)**

**Deep Dive:** This uses a **Downstream Collector**.
...
**(Source: Chapter 1 - Variables / Lesson 2)**
...
**Which lines cause a compilation error?** A. Line 1 and Line 4 B. Line 3 and Line 4 C. Line 1 and Line 3 D. All lines.

**Answer: A (Line 1 and Line 4)**
...
**What is the value of `name` after `Employee` is deserialized?** A. `Unknown` B. `Default` C. `null` D. The value it had during serialization.

**Answer: B (`Default`)**

**Deep Dive:** When deserializing an object:
...
**(Source: Chapter 2 - Flow Control / Lesson 3)**
...
**What happens?** A. Prints 5. B. Compilation Error: `i` not in scope. C. Runtime Error. D. Prints `null`.

**Answer: A (Prints 5)**

**Deep Dive:** This is **Flow Scoping** in Pattern Matching (Java 16+).
